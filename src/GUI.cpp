...
void Shapesdraw::drawNormalCommit(sf::Vector2f cir_center, float cir_radius)
{
...
void Shapesdraw::commitDraw()
{
	...
	{
		/*
		for (auto& iter : commit_id_map[detached_head->commit_id]->forward)
		{
			if (iter.second == NULL)
			{
		...
		*/
		this->circle[circle_count].setRadius(cir_radius);
		this->circle[circle_count].setFillColor(sf::Color::Green);
		this->cir_position[circle_count].x = checkedout_circle.getPosition().x;
		this->cir_position[circle_count].y = checkedout_circle.getPosition().y - 6 * cir_radius;
		circle[circle_count].setPosition(cir_position[circle_count]);
		std::cout << "checmout commit" << std::endl;

		this->line_position[line_count].x = cir_position[circle_count].x + cir_radius;
		this->line_position[line_count].y = cir_position[circle_count].y + 2 * cir_radius;
		this->line[line_count].setPosition(line_position[line_count]);
		...
		this->line_position[line_count].x = this->cir_position[circle_count].x + 2 * cir_radius;
		this->line_position[line_count].y = this->cir_position[circle_count].y + cir_radius;

		this->line[line_count].setPosition(line_position[line_count]);
		this->line[line_count].setSize(sf::Vector2f(40, 10));
		this->line[line_count].setFillColor(sf::Color::Red);

		tempcir_info.branch_circle = circle[circle_count];
		tempcir_info.branch = current_branch;

		circle_infos.push_back(tempcir_info);

		guiCommitid_shape[commitidVector.back()] = &circle[circle_count];

		circle_commitid[&circle[circle_count]] = commit_id_val;
	...
	this->circle[circle_count].setRadius(cir_radius);
	this->cir_position[circle_count].x = this->line[line_count - 1].getPosition().x + this->line[line_count - 1].getSize().x;// +circle[commit_count].getRadius();
	this->cir_position[circle_count].y = this->line[line_count - 1].getPosition().y - this->circle[circle_count].getRadius();
	this->circle[circle_count].setPosition(cir_position[circle_count]);
	this->circle[circle_count].setFillColor(sf::Color::Green);
...
void Shapesdraw::branchDraw()
{
	tempcir_info.branch_circle = circle[detached_head->commit_id];
	tempcir_info.branch = current_branch;
